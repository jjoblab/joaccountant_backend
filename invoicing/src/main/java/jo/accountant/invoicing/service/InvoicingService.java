...
public class InvoicingService {
    ...
    // Audit v4.7 §4.1 Finding #5 — Factur-X pour facturation électronique 2026
    ...
    // Finding #6 — TVA sur encaissement : port d'accès au VatMode (DEBIT/ENCAISSEMENT) défini sur
        ...
        if (hasRuleCode) {
            ...
            if (snapshot.isEmpty()) {
                throw new ValidationException("WITHHOLDING_RULE_NOT_FOUND",
                    ...
                    + "Codes Haïti pré-configurés (V64) : RS_HT_PRESTATIONS_LOCAL (2%), "
     ...
     *
     * <p><b>Finding #6 — TVA sur encaissement</b> : si le {@code TaxRule} applicable au taux
     ...
     *
     * <p><b>Finding #7 — Autoliquidation / reverse-charge (intra-UE B2B)</b> : si le tiers
     ...
     * du CGI ». La logique de TVA différée (Finding #6) est désactivée en autoliquidation (rien
    ...
    private void generateInvoiceEntry(UUID companyId, SalesInvoice invoice) {
        ...
        // ── Finding #7 — Autoliquidation / reverse-charge (intra-UE B2B) ──
        ...
        // collectée). On saute également la logique de TVA différée (Finding #6) — il n'y a
        ...
        // (compte 446 « État-TCA » créé par V60 pour PCN_HAITI — présent dans le plan comptable
        ...
        // fallback "448000"/"448" (compte 448 « État-taxes diverses » créé par V60 pour PCN_HAITI).
        ...
        // ── Finding #6 — TVA sur encaissement : déterminer le VatMode applicable ──
        ...
        // Note (Finding #7) : en autoliquidation, on force deferVat = false — la TVA n'est pas
        ...
        if (reverseCharge) {
            // ── Finding #7 — Crédit 447 « TDA autoliquidation » (au lieu de 443) ──
        ...
        // Mémoriser le montant de TVA différée pour le bascule au règlement (Finding #6).
        // v6-1 : seule la TVA peut être différée (la TCA et les autres taxes sont sur débits).
        invoice.setVatDeferredAmount(vatDeferred);
    }

    /**
     * Détermine si la TVA de la facture doit être différée (mode ENCAISSEMENT) — Finding #6.
     *
    ...
    public InvoiceResponse recordPayment(UUID companyId, UUID invoiceId, RecordPaymentRequest req) {
        ...
        // ── Finding #6 — TVA sur encaissement : bascule 4438 → 443 au règlement ──
        ...
        // Math correcte pour les paiements partiels successifs (voir Finding #6 dans la javadoc
    ...
    /**
     * Calcule le montant de TVA à basculer du 4438 vers le 443 lors d'un règlement — Finding #6.
     *
    ...
    /**
     * Poste l'écriture de bascule TVA (4438 → 443) au règlement — Finding #6.
     *
    ...
    public byte[] getInvoicePdf(UUID companyId, UUID invoiceId) {
        ...
        // ── Finding #7 — Mention légale autoliquidation (reverse-charge intra-UE B2B) ──
        ...
        jo.accountant.documentgeneration.dto.GeneratedDocumentResponse doc =
            documentGenerationService.generateDocument(
                companyId,
                jo.accountant.documentgeneration.entity.DocumentType.INVOICE,
                invoice.getId(),
                variables);
    ...
    /**
     * Génère le XML Factur-X BASICWL pour une facture — audit v4.7 §4.1 Finding #5.
     *
    ...
    public byte[] getInvoiceFacturX(UUID companyId, UUID invoiceId) {
        ...
        // (audit v4.7 §4.2 + Lot B R-21 — NIF Haïti sérialisé comme SpecifiedTaxRegistration).
        jo.accountant.company.entity.Company company = companyRepository.findById(companyId)
            .orElseThrow(() -> new NotFoundException("Company", companyId));
        FacturXExporter.TradeParty seller = new FacturXExporter.TradeParty(
            ...
            company.getAddress(),
            company.getCountry(),
            company.getNif()  // Lot B R-21 — NIF séparé pour conformité DGI Haïti
        );
        FacturXExporter.TradeParty buyer = new FacturXExporter.TradeParty(
            ...
            tp.getSiret(),
            tp.getAddress(),
            null,  // country du tiers non persisté — TODO v4.8
            tp.getNif()  // Lot B R-21 — NIF séparé pour conformité DGI Haïti
        );
    ...
    /**
     * Génère un PDF/A-3 avec le XML Factur-X embarqué (audit v4.7 §4.1 Finding #9).
     *
    ...
    public List<InvoiceResponse> listInvoices(UUID companyId) {
        // Audit v4.7 §7.2 Finding #5 — hard cap 200 pour empêcher l'OOM sur entreprises matures.
    ...
    public List<InvoiceResponse> listInvoices(UUID companyId, java.util.UUID fiscalYearId) {
        ...
        // Audit v4.7 §6.2 Finding #3 — IDOR CRITICAL : sans ce guard, un attaquant pouvait énumérer
    ...
    /**
     * Liste paginée des factures — Finding #3.
     *
