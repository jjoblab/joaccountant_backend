...
- `Employee` — fiche employé. Champs : `thirdPartyId` (FK vers `ThirdParty` de type
  ...
  **V49 (audit v4.7 §4.1 Finding #18)** : `overtimeHours25` (heures sup majorées +25 %),
  ...
  moteur de paie V40 pour calculer le brut ajusté.
...
- `src/main/resources/db/migration/V26__employees.sql` — table `employee`. CHECK sur
  `contract_type` et `status`. Index sur `(company_id, status)`, `third_party_id`.
  Contrainte unique `(company_id, employee_number)`.
- `src/main/resources/db/migration/V49__employees_overtime_absences.sql` — **V49 — Finding #18**.
