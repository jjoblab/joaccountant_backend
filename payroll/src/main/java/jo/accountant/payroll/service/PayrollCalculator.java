 ...
 * (audit v4.7 §4.1 Finding #3 — FIX CRITIQUE).
 *
 ...
 *
 * <h2>Finding #18 — Heures supplémentaires + absences + congés payés</h2>
...
public class PayrollCalculator {
    ...
    /** Finding #18 — nombre de jours travaillés par mois (convention mensuelle). */
    private static final BigDecimal WORKING_DAYS_PER_MONTH = new BigDecimal("30");

    /**
     * Finding #18 — durée légale mensuelle en heures (France 35h/sem × 52/12 ≈ 173.33h).
     * Utilisée pour calculer le taux horaire de référence : {@code hourlyRate = baseSalary / 173.33}.
     *
     * <p><b>Lot B R-20</b> — cette constante n'est plus utilisée pour le calcul réel : la
    ...
    /** Finding #18 — majoration HS +25% (coefficient 1.25). */
    private static final BigDecimal OVERTIME_25_RATE = new BigDecimal("1.25");

    /** Finding #18 — majoration HS +50% (coefficient 1.50). */
    private static final BigDecimal OVERTIME_50_RATE = new BigDecimal("1.50");

    /** Lot B R-20 — majoration HS +100% (coefficient 2.0) pour Haïti (au-delà de 56h/sem, dimanches/jours fériés). */
    private static final BigDecimal OVERTIME_100_RATE = new BigDecimal("2.00");

    /**
     * Lot B R-25 — Jackson pour parser {@code bracketsJson} des ContributionRule PROGRESSIVE
    ...
    /**
     * Constructeur par défaut — Lot B R-25 (injection ObjectMapper pour parser les bracketsJson
    ...
    /**
     * V78 — v7-6 : Constructeur avec OfatmaSectorRateRepository pour résolution dynamique
    ...
    /**
     * V78 — v7-6 : Résout le taux OFATMA Accidents pour un secteur donné.
     *
     * <p>Si le sectorCode est null/blank → taux défaut 2,00% (rétro-compat V57).
    ...
    public BigDecimal resolveOfatmaAccidentRate(String sectorCode) {
        if (sectorCode == null || sectorCode.isBlank()) {
            return new BigDecimal("2.00");  // défaut V57
        }
        if (ofatmaSectorRateRepository == null) {
            LOG.warn("V78 — OfatmaSectorRateRepository non injecté, fallback taux 2.00%");
    ...
    /**
     * Calcule les cotisations à partir de la fiche employé — Finding #18.
     *
     ...
     *   hourlyRate  = baseSalary / monthlyLegalHours   (173.33 France / 208 Haïti — Lot B R-20)
     *   overtime25  = overtimeHours25 × hourlyRate × 1.25
     *   overtime50  = overtimeHours50 × hourlyRate × 1.50
     *   overtime100 = overtimeHours100 × hourlyRate × 2.00   (Lot B R-20 — HS Haïti)
     ...
     *
     * <p><b>Lot B R-20</b> — la durée légale mensuelle est lue depuis {@code monthlyLegalHours}
     ...
     * @param monthlyLegalHours durée légale mensuelle (173.33 France, 208 Haïti — Lot B R-20).
    ...
    public PayrollCalculationResult calculate(UUID companyId, UUID employeeId,
        ...
        // Lot B R-20 — HS +100% pour Haïti (au-delà de 56h/sem, dimanches/jours fériés).
        BigDecimal overtimeHours100 = nz(employee.getOvertimeHours100());

        // Lot B R-20 — durée légale mensuelle configurable (France 173.33, Haïti 208).
        ...
        // --- V79 — v7-7 : Indemnité congés payés séparée du salaire de base ---
        ...
        // hourlyRate = baseSalary / monthlyLegalHours (Lot B R-20 — configurable)
        ...
        // Lot B R-20 — HS +100% = overtimeHours100 × hourlyRate × 2.00 (Haïti)
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("V79 — PayrollCalculator {} : base={} legalHours={} absences={} CP={} (indemnity={}) HS25={} HS50={} HS100={} → baseProRata={} brut={}",
        ...
        // V78 — v7-6 : Résolution dynamique du taux OFATMA Accidents par secteur.
        // Si l'employé a un ofatmaSectorCode renseigné, on surcharge le taux 2% (défault V57)
        ...
        if (employee.getOfatmaSectorCode() != null && !employee.getOfatmaSectorCode().isBlank()) {
            ...
            LOG.info("V78 — Employé {} sectorCode={} → taux OFATMA Accidents = {}%",
     ...
     * @param overtimeAmount100 montant des HS +100% en devise (Lot B R-20 — Haïti)
    ...
    private PayrollCalculationResult doCalculate(UUID employeeId,
        ...
        // V76 — v7-5 : Séparer les règles ITS des règles de cotisations sociales.
        ...
        for (ContributionRule rule : socialRules) {
            ...
            // V78 — v7-6 : Pour OFATMA_HT_ACCIDENT, surcharger le taux par le taux sectoriel
            ...
            if ("OFATMA_HT_ACCIDENT".equals(rule.getCode()) && ofatmaRateOverride != null) {
                ...
                LOG.debug("V78 — OFATMA_HT_ACCIDENT taux surchargé : {} → {}",
                    rule.getRate(), effectiveRate);
            }

            // Lot B R-25 — si bracketType=PROGRESSIVE, calculer le montant via bracketsJson
        ...
        // V76 — v7-5 : Calcul ITS sur l'assiette (brut - cotisations sociales salariales).
    ...
    /**
     * Lot B R-25 — Calcule le montant d'une cotisation à barème progressif (ex: AST Haïti).
     *
    ...
    private BigDecimal[] computeProgressiveAmount(BigDecimal assiette, String bracketsJson) {
        ...
        try {
            ...
            // Trier par threshold croissant (sécurité — les seeds V57 sont déjà triés)
     ...
     *
     * @param baseSalary           salaire de base brut (avant prorata) — Finding #18
     * @param baseProRata          salaire de base après prorata absences/CP — Finding #18
     * @param overtimeAmount25     montant des HS +25% en devise — Finding #18
     * @param overtimeAmount50     montant des HS +50% en devise — Finding #18
     * @param grossSalary          brut total = baseProRata + HS25 + HS50
    ...
    public record PayrollCalculationResult(
        ...
        /**
         * Constructeur de compatibilité 11-args (avant Lot B R-20 — sans overtimeAmount100).
         * Délègue avec overtimeAmount100=0.
         * @deprecated utiliser le constructeur 12-args avec overtimeAmount100 (Lot B R-20).
         */
        @Deprecated
    ...
    /**
     * V75 — v7-4 : Calcul du 13e mois (Code du Travail Haïti art. 153).
     *
    ...
    public BigDecimal calculateThirteenthMonth(jo.accountant.employees.entity.Employee employee, int year) {
        ...
        if (Boolean.FALSE.equals(employee.getThirteenthMonthEligible())) {
            LOG.info("V75 — Employé {} non éligible au 13e mois (thirteenthMonthEligible=false)",
                employee.getId());
        ...
        if (hireDate == null) {
            LOG.warn("V75 — Employé {} sans hireDate, 13e mois = 0", employee.getId());
            return BigDecimal.ZERO;
        }
        ...
        LOG.info("V75 — 13e mois employé {} (embauché le {}, {} mois travaillés en {}) : brut = {} (prorata {})",
    ...
    /**
     * V75 — v7-4 : Calcule le 13e mois net (après ITS, sans cotisations sociales).
     *
    ...
    // =========================================================================
    // V76 — v7-5 : ITS Haïti (Impôt Traitements / Salaires)
    // =========================================================================

    /**
     * V76 — v7-5 : Calcule l'ITS (Impôt sur Traitements et Salaires) sur un salaire imposable.
     *
    ...
    public BigDecimal computeIts(BigDecimal taxableBase, java.util.List<ContributionRule> itsRules) {
        ...
        if (itsRules == null || itsRules.isEmpty()) {
            LOG.warn("V76 — Aucune règle ITS configurée, retour 0");
            return BigDecimal.ZERO;
        }
        ...
        if (itsRule.getBracketType() != jo.accountant.core.tax.WithholdingBracketType.PROGRESSIVE) {
            ...
            LOG.info("V76 — ITS (FLAT) base={} rate={}→ montant={}",
        ...
        BigDecimal progressiveAmount = computeProgressiveAmount(taxableBase, itsRule.getBracketsJson())[0]
            .setScale(2, java.math.RoundingMode.HALF_UP);
        LOG.info("V76 — ITS (PROGRESSIVE) base={} brackets={} → montant={}",
            taxableBase, itsRule.getBracketsJson(), progressiveAmount);
        return progressiveAmount;
    }

    /**
     * V76 — v7-5 : Calcule l'assiette imposable ITS = brut - CNSS - OFATMA - AST.
     *
