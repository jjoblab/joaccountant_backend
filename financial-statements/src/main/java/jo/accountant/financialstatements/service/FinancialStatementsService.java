...
public class FinancialStatementsService {
    ...
    private BalanceSheet computeBalanceSheetFunctional(UUID companyId, LocalDate asOf) {
        ...
        // R-11 (lot-C-perf-devops) — agrégation SQL au lieu de charger toutes les lignes.
    ...
    private IncomeStatement computeIncomeStatementFunctional(UUID companyId, LocalDate from, LocalDate to) {
        List<Account> allAccounts = accountRepository.findByCompanyIdOrderByCode(companyId);
        // R-11 (lot-C-perf-devops) — agrégation SQL au lieu de findAllPostedBetweenDates
     ...
     *
     * <p><b>Audit v4.7 §3.1 Finding #9 — FIX</b> : après {@code AccountingEngineService.closeFiscalYear},
     ...
     *
     * <p><b>Audit v4.7 §3.1 Finding #5 — FIX</b> : obligatoire en IFRS (IAS 7) et SYSCOHADA,
    ...
    private jo.accountant.financialstatements.dto.CashFlowStatement computeCashFlowStatementFunctional(
        ...
        // R-11 (lot-C-perf-devops) — Réécriture avec agrégations SQL.
    ...
    /** R-11 — Calcule la variation de solde (débit - crédit) pour les comptes dont le code commence par `prefix`. */
    ...
    /** R-11 — Calcule le solde des comptes de trésorerie (5x : Banques 52, Caisse 57). */
    ...
    private BalanceSheet convertBalanceSheet(BalanceSheet functional,
        ...
        // V74 — v7-3 : CTA (Cumulative Translation Adjustment) — IAS 21
        ...
        LOG.info("V74 — v7-3 — CTA calculé : {} {} → {} {} (taux {}, CTA = {} {})",
    ...
    /**
     * V73 — v7-2 : SCE avec conversion de devise de présentation optionnelle (taux de clôture).
     */
    @Transactional(readOnly = true)
    public jo.accountant.financialstatements.dto.StatementOfChangesInEquity getStatementOfChangesInEquity(
            UUID companyId, LocalDate from, LocalDate to,
    ...
    BigDecimal treasuryShares = treasuryMovements.negate().max(BigDecimal.ZERO);
        ...
        LOG.info("V73 — SCE companyId={} from={} to={} : opening={} closing={} netIncome={} OCI={} capIssued={} treasury={} dividends={}",
    ...
    /**
     * V73 — v7-2 : Charge les écritures individuelles sur comptes de capitaux propres
