...
public interface PurchaseInvoiceLineRepository extends JpaRepository<PurchaseInvoiceLine, UUID> {
    ...
    /**
     * R-10 (lot-C-perf-devops) — Agrégation par taux de TVA côté SQL (côté achats).
     *
    ...
    @Query("select l.taxRate AS taxRate, " +
           ...
           "  and (:from is null or p.issueDate >= :from) " +
           "  and (:to is null or p.issueDate <= :to)" +
           ") " +
           "group by l.taxRate " +
           "order by l.taxRate")
    ...
    /**
     * R-10 (lot-C-perf-devops) — Projection pour l'agrégation SQL par taux de TVA (achats).
     */
