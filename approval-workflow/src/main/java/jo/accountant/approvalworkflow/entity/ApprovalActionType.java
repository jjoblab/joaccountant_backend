 ...
 * correspondent aux consommateurs identifiés dans le prompt maître :
 *
 * <ul>
 * <li>{@link #JOURNAL_ENTRY_POST} — consommé par {@code accounting-engine}* au postage d'une écriture dont le montant total dépasse le seuil configuré</li>
 * <li>{@link #INVOICE_ISSUE} — consommé par {@code invoicing}à l'émission
 * d'une facture dont le montant HT dépasse le seuil</li>
 * <li>{@link #GRANT_DISBURSEMENT_PROPOSAL} — consommé par {@code funds-grants}* à la proposition d'écriture de fonds dédiés</li>
 * </ul>
 *
