...
/**
 * Règles de complexité des mots de passe (§13 Phase 1 + audit v4.7 §6.3 + R-35 lot-F3-security).
 *
 * <p>Règles :
 * <ul>
 ...
 *   <li><b>R-35</b> : si HIBP activé ({@code app.password.hibp.enabled=true}), vérifie
 ...
 * </ul>
 *
 * <p><b>R-35 (lot-F3-security) — Intégration HIBP (Have I Been Pwned)</b> :
 * <ul>
...
public class PasswordValidator {
    ...
    public void validate(String password) {
        ...
        // R-35 (lot-F3-security) — HIBP check (activé uniquement si app.password.hibp.enabled=true)
        if (hibpEnabled) {
    ...
    void checkHibpBreaches(String password) {
        ...
        // R-35 — cache hit évite l'appel HTTP. Caffeine.get(key, mapping) exécute le mapping
        // uniquement si la key est absente du cache. Thread-safe (atomic compute).
        //
        // Note (lot-F2-tests-qa) : la signature de fetchHibpRange déclare `throws IOException,
