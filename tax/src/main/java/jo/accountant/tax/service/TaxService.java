...
public class TaxService {
    ...
    public TaxRule createTaxRule(UUID companyId, CreateTaxRuleRequest req) {
        ...
        rule.setActive(true);
        // Finding #6 — TVA sur encaissement : propager le mode d'exigibilité (DEBIT par défaut).
    ...
    public WithholdingRule createWithholdingRule(UUID companyId, CreateWithholdingRuleRequest req) {
        ...
        // ── Finding #14 — barème progressif (PAS FR) ──
     ...
     *
     * <p><b>R-10 (lot-C-perf-devops) — Réécriture en SQL GROUP BY</b> : l'ancien code faisait
     ...
     * (table {@code tax_credit_carried_forward} — R-23). Pour {@code taxType=VAT} ou {@code null},
    ...
    private TaxDeclaration getDeclarationLegacy(UUID companyId, LocalDate from, LocalDate to) {
        // ── Audit v4.7 §4.1 Finding #1 — FIX CRITIQUE : calcul de la TVA déductible (achats)
        ...
        // ── R-10 : TVA COLLECTÉE (factures de ventes ISSUED + PARTIALLY_PAID + PAID) ──
        ...
        // ── R-10 : TVA DÉDUCTIBLE (factures d'achats RECEIVED + PARTIALLY_PAID + PAID) ──
        // Audit v4.7 §4.1 Finding #1 — calcul de la TVA déductible (côté achats).
        // Une seule requête SQL GROUP BY au lieu de N+1.
        List<PurchaseInvoiceStatus> purchaseStatuses = List.of(
            PurchaseInvoiceStatus.RECEIVED, PurchaseInvoiceStatus.PARTIALLY_PAID, PurchaseInvoiceStatus.PAID);
        List<PurchaseInvoiceLineRepository.TaxRateAggregate> purchaseAggregates =
        ...
        // Lot B R-23 — le crédit est désormais PERSISTÉ en table tax_credit_carried_forward.
        ...
        // Lot B R-23 — persister le crédit pour la période suivante si > 0.
    ...
    /**
     * Lot B R-23 — Lit le crédit de TVA reporté pour une période donnée.
     *
     * <p>Recherche dans {@code tax_credit_carried_forward} la ligne correspondant à
     * {@code (companyId, VAT, year, month)}. Si aucune ligne n'est trouvée (première
     * déclaration de l'entreprise, ou pas de crédit reporté), retourne {@code ZERO}.
     *
     * <p>Le type de taxe est actuellement hardcoded à {@link TaxType#VAT} car la déclaration
     * TVA est la principale concernée. Une extension future gérera les crédits de TCA séparés.
     */
    private BigDecimal readCarriedForwardCredit(UUID companyId, int year, int month) {
        if (taxCreditCarriedForwardRepository == null) {
            LOG.warn("TaxCreditCarriedForwardRepository non injecté — fallback BigDecimal.ZERO");
            return BigDecimal.ZERO;
        }
        return taxCreditCarriedForwardRepository
            .findByCompanyIdAndTaxTypeAndPeriodYearAndPeriodMonth(
                companyId, jo.accountant.tax.entity.TaxType.VAT, year, month)
    ...
    /**
     * Lot B R-23 — Persiste (ou met à jour) le crédit de TVA pour une période donnée.
     *
     * <p>Si une ligne existe déjà pour {@code (companyId, VAT, year, month)}, on met à jour
    ...
    private void persistCarriedForwardCredit(UUID companyId, int year, int month,
        ...
        if (taxCreditCarriedForwardRepository == null) {
            LOG.warn("TaxCreditCarriedForwardRepository non injecté — crédit non persisté");
            return;
        }
        jo.accountant.tax.entity.TaxCreditCarriedForward credit = taxCreditCarriedForwardRepository
            ...
            .orElseGet(() -> {
                ...
                c.setTaxType(jo.accountant.tax.entity.TaxType.VAT);
        ...
        LOG.info("Crédit TVA persisté pour company {} période {}-{} : {} HTG",
            companyId, year, month, creditAmount);
    }

    // --- Impôt sur les Sociétés (IS) — audit v4.7 §4.1 Finding #4 ---
    ...
    // Lot B R-05 — repository Company pour déterminer le pays et appliquer la règle d'IS par défaut
    // correspondante (HT: 30%, FR: 25%, autre: 25% par défaut conservateur).
    private CompanyRepository companyRepository;
    // Lot B R-06 — calendrier déclaratif DGI Haïti (routing par pays).
    private HaitianTaxDeclarationSchedule haitianTaxDeclarationSchedule;
    // Lot B R-23 — repository des crédits de TVA reportés (remplace le BigDecimal.ZERO hardcoded).
     ...
     *
     * <p><b>Audit v4.7 §4.1 Finding #4 — FIX</b> : la v4.7 ne comportait aucun module IS.
    ...
    public CorporateTaxProjection projectCorporateTax(UUID companyId, LocalDate from, LocalDate to) {
        ...
        // 2. Récupérer la règle d'IS active (ou défaut par pays — Lot B R-05)
        // Lot B R-05 : la règle d'IS par défaut dépend du pays de la Company.
        ...
        BigDecimal charasseAddition = BigDecimal.ZERO;  // TODO v4.8 : calculer depuis les dividendes reçus
        BigDecimal otherAdditions = BigDecimal.ZERO;
        BigDecimal longTermCapitalGainDeduction = BigDecimal.ZERO;  // TODO v4.8 : plus-values LTPE
        ...
        // 6. IS net (crédits d'impôt à 0 par défaut — TODO v4.8 : saisie utilisateur)
    ...
    /**
     * Règle d'IS par défaut selon le pays de la Company (Lot B R-05).
     *
     * <p>Avant la R-05, cette méthode s'appelait {@code defaultFrenchCorporateTaxRule()} et
    ...
    private CorporateTaxRule defaultCorporateTaxRule(String countryCode) {
        ...
        // les règles persistées (qui ont toutes un country_code non null depuis V65).
    ...
    /**
     * Récupère le code pays (ISO 3166-1 alpha-2) d'une Company. Lot B R-05.
     *
     ...
     * </ol>
     *
     * <p>Si la règle ZF/ONG n'existe pas en base (DB non migrée V79), on construit une
    ...
    private CorporateTaxRule resolveCorporateTaxRule(UUID companyId, String countryCode) {
        ...
        if (company != null
            ...
            if (rule == null) {
                // Fallback défensif : règle ONG ad-hoc (DB non migrée V79)
                ...
                // Sécurité : forcer eligibility=NGO_EXEMPT même si la seed V65 l'avait omis
                rule.setEligibility(CorporateTaxEligibility.NGO_EXEMPT);
            }
        ...
        if (isFreeZone) {
            ...
            if (rule == null) {
                // Fallback défensif : règle ZF ad-hoc (DB non migrée V79)
                rule = new CorporateTaxRule();
                rule.setActive(true);
                ...
                // Sécurité : forcer eligibility=FREE_ZONE (V65 l'avait créée avec UNKNOWN)
                rule.setEligibility(CorporateTaxEligibility.FREE_ZONE);
            }
    ...
    public TaxDeclarationSchedule getDeclarationSchedule(UUID companyId, int year) {
        // Lot B R-06 — route vers le calendrier DGI Haïti si la Company est en Haïti.
     ...
     * Génère le planning annuel des échéances fiscales françaises (audit mobile #7).
     *
     * <p>Méthode historique conservée pour compat arrière (Lot B R-06). Le routing par pays
    ...
    public TaxDeclarationSchedule getFrenchDeclarationSchedule(UUID companyId, int year) {
        ...
        // TODO v4.8 : persister un vatRegime au niveau de l'entreprise (champ dédié).
     ...
     *   <li>{@code taxCreditCarriedForward} — 0 (pas de report de crédit RS pour l'instant ;
     *       TODO v6.3 : implémenter le report si la somme des avoirs dépasse celle des factures
     *       sur la période — cas rare mais possible).</li>
     *   <li>{@code taxDue} — total RS à reverser à la DGI pour le compte des clients
     *       (égale à {@code totalTaxCollected}, plancher à 0).</li>
     *   <li>{@code taxCreditToCarryForward} — part négative (si avoirs > factures sur la période)
     *       à reporter sur la déclaration RS du mois suivant.</li>
     * </ul>
     *
    ...
    public TaxDeclaration getWithholdingDeclaration(UUID companyId, LocalDate from, LocalDate to) {
        ...
        // taxDue = max(0, totalWithholding) — si les avoirs > factures sur la période,
        // le solde négatif devient un crédit à reporter sur la déclaration RS du mois suivant.
        BigDecimal taxDue;
        BigDecimal taxCreditToCarryForward;
        if (totalWithholding.compareTo(BigDecimal.ZERO) >= 0) {
            taxDue = totalWithholding;
            taxCreditToCarryForward = BigDecimal.ZERO;
        } else {
            taxDue = BigDecimal.ZERO;
            taxCreditToCarryForward = totalWithholding.negate();
        }

        // Pas de report de crédit lu pour l'instant (TODO v6.3 — table dédiée tax_credit_carried_forward
        // étendue au type RS, ou réutilisation de l'existant avec TaxType.WITHHOLDING).
        BigDecimal taxCreditCarriedForward = BigDecimal.ZERO;

        LOG.info("Déclaration RS {} [{} à {}] : {} factures avec RS, {} taux, total RS = {} HTG, dû = {}, crédit à reporter = {}",
            companyId, from, to, invoicesWithRs.size(), byRate.size(),
            totalWithholding, taxDue, taxCreditToCarryForward);
