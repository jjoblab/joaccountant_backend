...
- `TaxRule` — règle de TVA. Champs : `code` (unique par entreprise), `label`, `rate`
  ...
  `active`. **V44** : `vatMode` (`DEBIT` défaut — exigible à l'émission, ou `ENCAISSEMENT` —
...
- `WithholdingRule` — règle de retenue à la source. Champs : `code`, `label`, `rate`,
  ...
  `applicableFrom`, `applicableTo`, `active`. **V46** : `bracketType` (`FLAT` défaut, ou
...
- `ContributionRule` — **V40 — audit v4.7 §4.1 Finding #3** — règle de cotisation sociale
...
- `VatMode` (enum, V44) — `DEBIT`, `ENCAISSEMENT`.
- `WithholdingBracketType` (enum, V46) — `FLAT`, `PROGRESSIVE`.
- `ContributionBase` (enum, V40) — `GROSS`, `GROSS_ABATED`, `CAPPED_GROSS`,
  `CAPPED_GROSS_ABATED`, `TRANCHE_B`.
- `ContributionRegime` (enum, V40) — `FR_GENERAL`, `FR_CADRE`, `FR_NON_CADRE`, `HT_GENERAL`,
  `CUSTOM`.
- `ContributionType` (enum, V40) — `EMPLOYEE`, `EMPLOYER`, `EMPLOYEE_AND_EMPLOYER`.
...
|---|---|---|---|
| POST | `/api/v1/companies/{companyId}/tax/rules` | Crée une règle de TVA (avec `vatMode` V44 optionnel, défaut `DEBIT`) | 422 `TAX_RULE_CODE_REQUIRED`/`RATE_INVALID` |
| GET | `/api/v1/companies/{companyId}/tax/rules` | Liste les règles de TVA (entreprise + globales) | — |
| POST | `/api/v1/companies/{companyId}/tax/withholding-rules` | Crée une règle de retenue à la source (`bracketType=FLAT` ou `PROGRESSIVE` + `brackets`, V46) | 422 champs invalides |
...
| GET | `/api/v1/companies/{companyId}/tax/corporate-tax/projection?from=&to=` | **V40 — audit v4.7 §4.1 Finding #4** — Projection IS (15 % PME / 25 %, 4 acomptes + solde 15 mai N+1). Si aucune règle IS configurée, utilise les valeurs par défaut France 2026. | 422 dates invalides |
...
**V40 — audit v4.7 §4.1 Finding #3**. CRUD des règles de cotisation sociale pour
...
- `:payroll` — **`PayrollCalculator`** (V40) consomme `ContributionRule` pour calculer les
...
- `src/main/resources/db/migration/V18_001__tax.sql` — tables `tax_rule` et
...
- `src/main/resources/db/migration/V40__contribution_rule.sql` — **V40 — audit v4.7 §4.1 #3**.
...
- `src/main/resources/db/migration/V44__vat_mode_encaissement.sql` — **V44 — Finding #6**.
...
- `src/main/resources/db/migration/V46__withholding_rule_progressive_brackets.sql` —
  **V46 — Finding #14**. Ajoute `bracket_type` (`FLAT`/`PROGRESSIVE`) et `brackets` (JSONB
  `[{threshold, rate}]`) sur `withholding_rule`. Défaut `FLAT` (rétro-compat).
- `src/main/resources/db/migration/V50__tax_accounts_seed.sql` — **V50 — Finding #10**. Seed
  des comptes 447 (TVA autoliquidation / reverse charge) et 4438 (TVA différée non encaissée)
  dans les plans SYSCOHADA et PCG_FRANCE (SectorAccountTemplate). Avant V50, ces comptes
  étaient référencés par `InvoicingService` mais jamais seedés — la première facture en
  autoliquidation (V45) ou en TVA encaissement (V44) levait `VAT_REVERSE_CHARGE_ACCOUNT_NOT_FOUND`
...
`business_type_module` inclut `TAX` (voir `V3_003__business_type.sql`).
