 ...
 * {@code AccountingEngineService} (Finding #17 — refactor batch C).
 *
 * <p><b>Motivation</b> : {@code AccountingEngineService} était encore un God class de 1313 lignes
 * après l'extraction de {@code FiscalYearClosingService} (Finding #2 batch 2). Il mêlait
...
public class JournalEntryLifecycleService {
     ...
     *
     * <p><b>Audit v4.7 §3.1 Finding #2 — FIX</b> :
     * <ul>
     ...
     *       v4.7 §3.1 Finding #4 notait que ce cas était accepté, créant des chaînes illisibles.</li>
     * </ul>
     *
    ...
    public JournalEntryResponse reverseJournalEntry(UUID companyId, UUID entryId, String reason,
        ...
        // Audit v4.7 §3.1 Finding #4 — empêcher reversal de reversal
        ...
        // Audit v4.7 §3.1 Finding #2 — date paramétrable + vérification période OPEN
    ...
    private void validateAnalyticalTags(UUID companyId, Account account, JournalLine line) {
        ...
        for (JournalLineAnalyticalTag tag : tags) {
            ...
            // Audit v4.7 §6.2 Finding #3 — IDOR CRITICAL : valider que le plan appartient à la company
